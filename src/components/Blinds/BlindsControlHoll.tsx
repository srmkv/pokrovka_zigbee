...
const BlindsControlHoll: React.FC = () => {
  const { position, setBlinds, loading } = useBlindsPosition("holl");
  const [sliderValue, setSliderValue] = useState<number>(position);

  // Синхронизируем ползунок при изменении с сервера
  useEffect(() => {
    setSliderValue(position);
  }, [position]);

  const handleApply = () => setBlinds(sliderValue);
  const handleUp = () => {
    setSliderValue(0);
    setBlinds(0);
  };
  ...
  return (
    <div className="bg-[#22243c] rounded-xl p-6 flex flex-col items-center w-full">
      <h4 className="text-lg font-semibold mb-3">Жалюзи Зал</h4>
      <div
...
export default BlindsControlHoll;
