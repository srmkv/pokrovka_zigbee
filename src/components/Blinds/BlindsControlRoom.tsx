...
import { useBlindsPosition } from "../../hooks/useBlindsPosition"; // поправь путь, если надо
...
const BlindsControlRoom: React.FC = () => {
  // позиция — число 0…100
  const { position, setBlinds, loading } = useBlindsPosition("room");
  const [sliderValue, setSliderValue] = useState<number>(position);

  // Синхронизируем sliderValue при изменении position с сервера
  useEffect(() => {
  ...
  return (
    <div className="bg-[#22243c] rounded-xl p-6 flex flex-col items-center w-full">
      <h4 className="text-lg font-semibold mb-3">Жалюзи Комната</h4>
      <div
      ...
      <span className="text-gray-350 text-xs mt-2">
        Управляйте жалюзи с помощью кнопок или ползунка
      </span>
    </div>
  );
};

export default BlindsControlRoom;
