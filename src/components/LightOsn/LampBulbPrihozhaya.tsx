...
const LampBulbPrihozhaya: React.FC = () => {
  const [isOn, setIsOn] = useState(false);
  const [loading, setLoading] = useState(false);

  // Инициализация: читаем текущее состояние из /api/home → relays.prihozhaya
  useEffect(() => {
    (async () => {
      try {
        ...
        if (j?.relays && typeof j.relays.prihozhaya === "boolean") {
          setIsOn(j.relays.prihozhaya);
        }
      } catch {}
    })();
  }, []);
  ...
  const toggleLamp = async () => {
    ...
    try {
      const response = await fetch(apiUrl("/relay/send-multiple"), {
        ...
        body: JSON.stringify({
          codes: [{ tag: "prihozhaya", code: RELAY_CODE, state: next }],
        }),
      });

      const result = await response.json().catch(() => ({}));
      if (!(response.ok && result?.sent?.[0]?.success)) {
        setIsOn(!next); // роллбэк при ошибке
        console.error("Ошибка ответа сервера:", result);
      }
    } catch (error) {
      setIsOn(!next); // роллбэк при сетевой ошибке
...
export default LampBulbPrihozhaya;
