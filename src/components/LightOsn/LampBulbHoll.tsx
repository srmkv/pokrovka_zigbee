...
const LampBulbHoll: React.FC = () => {
  const [isOn, setIsOn] = useState(false);
  const [loading, setLoading] = useState(false);

  // Подтягиваем стартовое состояние из /api/home → relays.holl
  useEffect(() => {
    (async () => {
      try {
        ...
        if (j?.relays && typeof j.relays.holl === "boolean") setIsOn(j.relays.holl);
      } catch {}
    })();
  }, []);
  ...
  const toggleLamp = async () => {
    ...
    // оптимистично переключим и заблокируем повторные клики
    setLoading(true);
    setIsOn(next);
    ...
    try {
      const response = await fetch(apiUrl("/relay/send-multiple"), {
        ...
        body: JSON.stringify({
          codes: [{ tag: "holl", code: RELAY_CODE, state: next }],
        }),
      });
  ...
  return (
    ...
    >
      ...
      <span style={{ color: isOn ? LAMP_ON : "#888" }}>Зал</span>
      <div
        style={{
          marginTop: 6,
          fontSize: 14,
