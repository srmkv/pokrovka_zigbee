...
const DishwasherSensor: React.FC = () => {
  const { dishwasherSensor = "unknown", lastLeakDishwasher } = useLeakSensorsState();
  ...
  if (dishwasherSensor === "leak") {
    color = "#e53e3e";
    ringClass = "bg-red-500 animate-ping";
  }
  if (dishwasherSensor === "unknown") {
    color = "#9ca3af";
  ...
  async function handleReset() {
    ...
    try {
      const resp = await fetch(`${API_BASE}/dishwasher`, {
        method: "POST",
  ...
  return (
    <>
      ...
      <ResetConfirmModal
        open={confirmOpen}
        busy={resetPending}
        title="Сбросить датчик у посудомойки?"
        description="После подтверждения тревога у датчика посудомойки будет переведена в нормальное состояние."
